...
> **轻量级局域网远程桌面工具** — 类似 VNC / Radmin，采用自定义轻量级协议，默认 TCP 可靠传输，可选扩展 UDP 等通道。提供屏幕共享、远程键鼠操控、共享剪贴板等能力。
>
> **当前实现**：.NET 8 控制台服务端 + WinForms 客户端，Deflate 压缩，局域网内可看可操控。
> **计划**：.NET Framework 4.0 + WPF（兼容 Windows XP ~ 11）和 .NET 8 + Avalonia（面向 Windows 7+ 及跨平台扩展）。
>
...
| 特性 | 说明 |
|---|---|
| 自定义轻量级协议 | 非标准 RDP/VNC，按需精简字段，降低协议开销 |
| 可扩展传输层 | 默认 TCP 可靠传输；抽象 `ITransportChannel` 接口，可扩展 UDP / WebSocket / NamedPipe 等 |
| 屏幕捕获 | 全屏 / 区域 / 指定显示器截图（BGRA32 原始像素），Deflate 实时压缩 |
| 远程输入注入 | 鼠标移动/点击/滚轮、键盘按键、Unicode 文本发送 |
| 剪贴板同步 | 远程→本地单向同步（本地→远程计划中） |
| 多显示器支持 | 虚拟桌面坐标空间，可切换 / 拼接多屏画面 |
| 全系统兼容 | 计划双版本：.NET 4 + WPF（兼容 XP ~ Win11）、.NET 8 + Avalonia（面向 Win7+） |
| 零依赖 | 纯 P/Invoke 调用 user32/gdi32/kernel32，不依赖 WinForms/WPF |
...
│  ┌────────┴─────────────────────┴────────┐ │
│  │       (当前) Console Server +         │ │
│  │       WinForms Client (.NET 8)  ✅    │ │
│  └──────────────────┬───────────────────┘ │
│  ┌──────────────────┴───────────────────┐ │
│  │        EasyRDP.Core 共享协议层         │ │
│  │  消息编解码 / CompressHelper / 传输     │ │
│  └──────────────────┬───────────────────┘ │
│  ┌──────────────────┴───────────────────┐ │
│  │         自定义轻量级协议层              │ │
│  │   ITransportChannel 可扩展传输层       │ │
│  │   ┌──────────┐  ┌──────────────────┐ │ │
│  │   │   TCP    │  │  UDP / WS / ...  │ │ │
│  │   │ (默认)   │  │  (可选扩展)       │ │ │
│  │   └──────────┘  └──────────────────┘ │ │
...
- **当前实现**：.NET 8 控制台服务端 + WinForms 客户端，Deflate 压缩，局域网可用
- **双 UI 策略（计划）**：.NET 4 + WPF 版本兼容 Windows XP ~ 11；.NET 8 + Avalonia 版本面向 Windows 7+ 及跨平台扩展，两套 UI 共享 `EasyRDP.Core`
- **EasyRDP.Core**：协议 + 传输层共享库，多目标 `net40;net8.0`，包含消息编解码、Deflate 压缩、连接管理、剪贴板同步。抽象 `ITransportChannel` 接口支持可扩展传输通道
...
- **传输层**：`ITransportChannel` 抽象接口，默认 `TcpChannel`；可选 `TcpAndUdp` 模式启用 UDP；通过 `TransportMode` 枚举切换
...
|---|---|---|---|---|---|---|
| **Windows 7+** | ✅ BitBlt | ✅ SendInput | ✅ GetCursorInfo | ✅ OpenClipboard | ✅ GetSystemMetrics | ✅ Console+WinForms |
| **Windows XP / 7** | ✅ BitBlt | ✅ SendInput | ✅ GetCursorInfo | ✅ OpenClipboard | ✅ GetSystemMetrics | ⏳ WPF 计划 |
| Linux (X11) | ⏳ 计划中 | ⏳ 计划中 | ⏳ 计划中 | ⏳ 计划中 | ⏳ 计划中 | ⏳ 计划中 |
...
│   │   └── Transport/           # ITransportChannel + Tcp/UdpChannel + Server/Client
│   ├── EasyRDP.Server/          # 控制台服务端 (.NET 8) ✅
│   ├── EasyRDP.Client/          # WinForms 客户端 (.NET 8) ✅
...
| **Phase 3** | 可扩展传输层 — TCP 默认 + ITransportChannel 抽象，连接管理、心跳、重连 | ✅ 已完成 |
| **Phase 4** | 屏幕编解码 — Deflate 压缩 ✅，增量编码 / 脏矩形检测 / 分块传输 ⏳ | ⚠️ 部分完成 |
| **Phase 5** | 剪贴板同步 — 远程→本地 ✅，本地→远程 + 格式协商 ⏳ | ⚠️ 部分完成 |
| **Phase 6** | 服务端 / 客户端 — 控制台 + WinForms ✅，WPF + Avalonia 版 ⏳ | ⚠️ 部分完成 |
...
| 版本 | 框架 | UI | 目标系统 | 定位 | 状态 |
|---|---|---|---|---|---|
| .NET 8 | WinForms | Windows Forms | Windows 7+ | **当前实现** | ✅ |
| .NET Framework 4.0 | WPF | WPF | Windows XP ~ 11 | 兼容老旧系统（XP 工控机、ATM 等） | ⏳ |
| .NET 8 | Avalonia | 跨平台 XAML | Windows 7+, Linux, macOS | 新功能主线，未来跨平台基础 | ⏳ |
